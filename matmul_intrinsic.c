...
int vecAddIntrin(int n, double *A, double *B){
	int NUM_ELEMENTS=SIZE*SIZE;
	size_t N_pd=(NUM_ELEMENTS*8)/sizeof(double);
	double *C = (double*)_mm_malloc(N_pd*sizeof(double), 32);
	//initialize(C, NUM_ELEMENTS, 0.0);
	...
	for(k=0;k<n;k=k+4){
		__m256d c0 = _mm256_load_pd(C+k);

		__m256d addC = _mm256_add_pd(c0, c0);

		_mm256_store_pd(C+k, c0);	
	}
...
void vecAddV2(int n, double *A, double *B, double *C){
	...
	for(i=0;i<n;i=i+4){
		...
		for(j=0;j<n;j++){
			c0=_mm256_add_pd(c0, 
				_mm256_mul_pd(a0, _mm256_broadcast_sd(B+j)));
		}
		_mm256_store_pd(C+i, c0);		
	}
}
...
int matmul_intrinsics(){
	int NUM_ELEMENTS = SIZE*SIZE;
	int dimension = SIZE;
	printf("dimension = %d & NUM_ELEMENTS %d, sizeof(double)=%1u \n ", dimension, 
	...
	vecAddV2(dimension, data_A, data_B, data_C);
...
int matmul(){
	...
	printf("Normal: numOps = %f GFLOPS %f \n", numOps, gflops);
